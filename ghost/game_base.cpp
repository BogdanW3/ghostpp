...
{
	string Description = m_GameName + " : " + m_OwnerName + " : " + UTIL_ToString( GetNumHumanPlayers( ) ) + "/" + UTIL_ToString( m_GameLoading || m_GameLoaded ? m_StartPlayers :((unsigned long) m_Slots.size( ) ));
	...
	if( GetTime( ) - m_LastPingTime >= 5 )
	{
		...
		if( !m_CountDownStarted )
		{
			...
			if( m_SaveGame )
			{
				...
				MapHeight.push_back( 0 );
				if (m_GHost->m_LANWar3Version <= 29)
			...
			else
			{
				...
				if (m_GHost->m_LANWar3Version <= 29)
					...
					m_GHost->m_Bonjour->Broadcast_Info(m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray(MapGameType, false), m_Map->GetMapGameFlags(), m_Map->GetMapWidth(), m_Map->GetMapHeight(), m_GameName, "Varlock", m_CreationTime, m_Map->GetMapPath(), m_Map->GetMapCRC(), MAX_SLOTS, m_Map->GetMapNumPlayers(), m_HostPort, FixedHostCounter, m_EntryKey, m_Map->GetMapHash());
			}
		}
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	Player->Send( m_Protocol->SEND_W3GS_MAPCHECK( m_Map->GetMapPath( ), m_Map->GetMapSize( ),
		m_Map->GetMapInfo( ), m_Map->GetMapCRC( ), ((m_GHost->m_LANWar3Version < 30) ? m_Map->GetMapSHA1() : m_Map->GetMapHash())));
...
void CBaseGame :: EventGameStarted( )
{
	...
	StatString.push_back( 0 );
	UTIL_AppendByteArray( StatString, 
		((m_GHost->m_LANWar3Version < 30) ? m_Map->GetMapSHA1() : m_Map->GetMapHash( )));
	// note: in replays generated by Warcraft III it stores 20 zeros for the SHA1 instead of the real thing
