...
{
	std::string Description = m_GameName + " : " + m_OwnerName + " : " + UTIL_ToString( GetNumHumanPlayers( ) ) + "/" + UTIL_ToString( m_GameLoading || m_GameLoaded ? m_StartPlayers :((unsigned long) m_Slots.size( ) ));
...
void CBaseGame :: SendWelcomeMessage( CGamePlayer *player )
{
	...
	if( in.fail( ) )
	{
		...
		SendChat( player, " " );
		SendChat( player, " " );
		SendChat( player, "GHost++ BogdanW3                                http://www.codelain.com/" );
		SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	Player->Send( m_Protocol->SEND_W3GS_MAPCHECK( m_Map->GetMapPath( ), m_Map->GetMapSize( ),
		m_Map->GetMapInfo( ), m_Map->GetMapCRC( ), ((!m_GHost->m_LANBonjour) ? m_Map->GetMapSHA1() : m_Map->GetMapHash())));
...
void CBaseGame :: EventGameStarted( )
{
	...
	UTIL_AppendByteArray( StatString, 
		((!m_GHost->m_LANBonjour) ? m_Map->GetMapSHA1() : m_Map->GetMapHash( )));
...
void CBaseGame :: ShuffleSlots( )
{
	...
	else
	{
		...
		shuffle( PlayerSlots.begin( ), PlayerSlots.end( ), g);
	}
