...
CMDNSSocket::CMDNSSocket() : CSocket()
{
	Allocate(SOCK_DGRAM);
	...
	setsockopt(m_Socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const char*)&imr, sizeof(ip_mreq));

	// set default broadcast target
	//m_BroadcastTarget.s_addr = INADDR_BROADCAST;
}

CMDNSSocket :: ~CMDNSSocket()
{

}

bool CMDNSSocket::SendTo(struct sockaddr_in sin, BYTEARRAY message)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
...
bool CMDNSSocket::SendTo(string address, uint16_t port, BYTEARRAY message)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
	...
	if (!HostInfo)
	{
		m_HasError = true;
		// m_Error = h_error;
		CONSOLE_Print("[MDNSSOCKET] error (gethostbyname)");
		return false;
	}
...
bool CMDNSSocket::Broadcast(BYTEARRAY message)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
	...
	if (sendto(m_Socket, MessageString.c_str(), MessageString.size(), 0, (struct sockaddr*) & sin, sizeof(sin)) == -1)
	{
		CONSOLE_Print("[MDNSSOCKET] failed to broadcast packet (port " + UTIL_ToString(5353) + ", size " + UTIL_ToString((unsigned long)MessageString.size()) + " bytes)");
		return false;
	}
...
/*void CMDNSSocket::SetBroadcastTarget(string subnet)
{
	if (subnet.empty())
	{
		CONSOLE_Print("[UDPSOCKET] using default broadcast target");
...
/*void CMDNSSocket::SetDontRoute(bool dontRoute)
{
	int OptVal = 0;
...
CMDNSServer::CMDNSServer() : CMDNSSocket()
{
	// make socket non blocking
...
bool CMDNSServer::Bind(struct sockaddr_in sin)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
	...
	if (::bind(m_Socket, (struct sockaddr*) & m_SIN, sizeof(m_SIN)) == SOCKET_ERROR)
	{
		m_HasError = true;
		m_Error = GetLastError();
		CONSOLE_Print("[MDNSSERVER] error (bind) - " + GetErrorString());
		return false;
	}
...
bool CMDNSServer::Bind(string address, uint16_t port)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
...
void CMDNSServer::RecvFrom(fd_set* fd, struct sockaddr_in* sin, string* message)
{
	if (m_Socket == INVALID_SOCKET || m_HasError || !sin || !message)
		return;
	...
	if (FD_ISSET(m_Socket, fd))
	{
		...
		else if (c == SOCKET_ERROR && GetLastError() != EWOULDBLOCK)
		{
			...
			m_HasError = true;
			m_Error = GetLastError();
			CONSOLE_Print("[MDNSSERVER] error (recvfrom) - " + GetErrorString());
		}
	}
}
