...
void CMap::Load(CConfig* CFG, std::string nCFGFile)
{
					...
					if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead, nullptr) )
					{
						...
						if( FileFormat == 18 || FileFormat == 25 || FileFormat == 28 || FileFormat == 31)
						{
							...
							if( FileFormat >= 25 )
							{
								...
								if (FileFormat >= 28)
								{
									ISS.seekg(4, std::ios :: cur);			// LUA toggle
									if (FileFormat >= 31)
										ISS.seekg(8, std::ios :: cur);		// Supported modes and game data
								}
							}
	...
	if (MapHash.empty())
		MapHash = UTIL_ExtractNumbers(CFG->GetString("map_hash", std::string()), 20);
	else if (CFG->Exists("map_hash"))
	{
		CONSOLE_Print("[MAP] overriding calculated map_hash with config value map_hash = " + CFG->GetString("map_hash", std::string()));
		MapHash = UTIL_ExtractNumbers(CFG->GetString("map_hash", std::string()), 20);
	}
