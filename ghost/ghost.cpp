...
CGHost :: CGHost( CConfig *CFG )
{
	...
	m_Bonjour = new CBonjour(CFG->GetString( "admingame_interface", "Network"));
	...
	m_LANWar3Version = CFG->GetInt( "lan_war3version", 31 ) ;
	m_LANBonjour = CFG->GetInt("lan_bonjour", 1) != 0;
	m_ReplayWar3Version = CFG->GetInt( "replay_war3version", 31 );
	m_ReplayBuildNumber = CFG->GetInt( "replay_buildnumber", 6072 );
	SetConfigs( CFG );
	...
	for( uint32_t i = 1; i < 10; ++i )
	{
		...
		unsigned char War3Version = CFG->GetInt( Prefix + "custom_war3version", 31 );
	...
	// extract common.j and blizzard.j from Warcraft if we can
			...
			if (CascOpenFile(Casc, ((m_LANWar3Version == 30) ? "war3.mpq:Scripts\\common.j" : "war3.w3mod:Scripts\\common.j"), 0, CASC_OPEN_BY_NAME, &SubFile))
			{
				...
				if (FileLength > 0 && FileLength != 0xFFFFFFFF)
				{
					...
					if (CascReadFile(SubFile, SubFileData, FileLength, &BytesRead))
					{
						...
						std::string Path = m_MapCFGPath + std::to_string(m_LANWar3Version) + "\\common.j";

						std::filesystem::create_directories(std::filesystem::path(Path).parent_path());
						UTIL_FileWrite(Path, (unsigned char*)SubFileData, BytesRead);
					}
					else
				...
				CascCloseFile(SubFile);
			}
			else
				CONSOLE_Print("[GHOST] couldn't find Scripts\\common.j in CASC");
			...
			if (CascOpenFile(Casc, m_LANWar3Version == 30 ? "war3.mpq:Scripts\\blizzard.j" : "war3.w3mod:Scripts\\blizzard.j", 0, CASC_OPEN_BY_NAME, &SubFile))
			{
				...
				if (FileLength > 0 && FileLength != 0xFFFFFFFF)
				{
					...
					if (CascReadFile(SubFile, SubFileData, FileLength, &BytesRead))
					{
						...
						std::string Path = m_MapCFGPath + std::to_string(m_LANWar3Version) + "\\blizzard.j";
				...
				CONSOLE_Print("[GHOST] couldn't find Scripts\\blizzard.j in CASC");

			CascCloseStorage(Casc);
		}
		else
			CONSOLE_Print("[GHOST] warning - unable to load CASC - error code " + UTIL_ToString(GetLastError()));
	}
	else
	{
